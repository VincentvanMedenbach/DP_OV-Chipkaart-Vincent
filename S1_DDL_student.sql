...
-- ADD geslacht varchar;
-- ALTER TABLE medewerkers ADD CHECK (geslacht = 'M' OR geslacht = 'V');
-- DETAIL:  Failing row contains (7934, MOLENAAR, TJA, BOEKHOUDER, 7782, 1982-01-23, 1300.00, null, 10, A).
...
--
--
--INSERT INTO medewerkers" ("mnr", "naam", "voorl", "functie", "gbdatum", "maandsal") VALUES ('8000', 'DONK', 'A', 'DIRECTEUR', '2021-09-10', '0');
...
--     ALTER COLUMN mnr SET DEFAULT nextval('afdeling_seq'::regclass);
--   b) Voeg een aantal afdelingen toe aan de tabel, maak daarbij gebruik van
--      de nieuwe sequence.
---- INSERT INTO afdelingen" ("naam", "locatie", "hoofd") VALUES ( 'ONDERZOEK', 'UTRECHT', '7900');
-- INSERT INTO afdelingen" ("naam", "locatie", "hoofd") VALUES ( 'ONDERZOEK', 'UTRECHT', '7900');
...
--     ALTER COLUMN anr TYPE numeric(20, 0);
...
CREATE TABLE "adressen" (
	...
	CONSTRAINT "FK__medewerkers" FOREIGN KEY ("med_mnr") REFERENCES "medewerkers" ("mnr")
);
ALTER TABLE adressen ADD CHECK (ingangsdatum < einddatum);
 CREATE UNIQUE INDEX telefoonunique ON adressen (telefoon);
ALTER TABLE adressen ADD CONSTRAINT
    postcode_constraint CHECK (postcode ~ '[0-9][0-9][0-9][0-9][A-Za-z][A-Za-z]');
...
ALTER TABLE medewerkers ADD CONSTRAINT medewerker_bedrag_check CHECK ((functie = 'VERKOPER' AND comm IS NOT NULL) OR (functie != 'VERKOPER' AND comm IS NULL))
