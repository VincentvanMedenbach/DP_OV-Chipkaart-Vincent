...
SELECT cursus, begindatum, cursussen.lengte, medewerkers.naam
FROM uitvoeringen
         INNER JOIN medewerkers on medewerkers.mnr = uitvoeringen.docent
         INNER JOIN cursussen on cursussen.code = uitvoeringen.cursus;
...
CREATE
OR REPLACE VIEW s3_4 AS                                                     -- [TEST]
SELECT medewerkers.naam, afdelingen.naam AS afdeling, afdelingen.locatie
FROM medewerkers
         INNER JOIN afdelingen ON afdelingen.anr = medewerkers.afd;
...
FROM medewerkers
         INNER JOIN schalen
                    ON (schalen.ondergrens <= medewerkers.maandsal AND schalen.bovengrens >= medewerkers.maandsal);
